import{p as H}from"./chunk-GW4FGG42-BSxs48Y2.js";import{A as N,k as E,p as J}from"./index-BISvRqRP.js";var y=null;async function _B(){return y??(y=K())}async function K(){return H(await J.fto.kpuzzle(),`SubgroupSizes 24 12 11 12 12 11 10 9 11 8 10 9 7 8 10 10 6 9 8 5 7 6 5 4 4 8 7 3 6 6 360
...
`)}function h(F){if(F<2)return F;let B=1;for(let L=2;L<=F;L++)B*=L;return B}function m(F,B){if(B<0||B>F)return 0;if(B===0||B===F)return 1;let L=1;for(let R=0;R<B;R++)L=L*(F-R)/(R+1)|0;return L}function P(F){F=F.slice();let B=F.length,L=h(B-1),R=0;for(;B>1;){B--;let l=F[0];R+=l*L;for(let U=0;U<B;U++){let g=F[U+1];F[U]=g-(g>l)}L/=B}return R}function Q(F,B){let L=[],R=h(B-1);for(let l=0;l<B;l++)L[l]=F/R|0,F%=R,R/=B-1-l;for(let l=B-2;l>=0;l--)for(let U=l+1;U<B;U++)L[U]+=+(L[U]>=L[l]);return L}function b(F){let B=F.length,L=0;for(let R=0;R<B-1;R++)for(let l=R;l<B;l++)F[R]>F[l]&&(L^=1);return L}function V(F,B){let L=[],R=h(B-1)/2,l=0;for(let U=0;U<B-1;U++)L[U]=F/R|0,F%=R,R/=B-1-U;L[B-1]=0;for(let U=B-2;U>=0;U--)for(let g=U+1;g<B;g++)L[g]>=L[U]?L[g]++:l^=1;return l===1&&([L[B-2],L[B-1]]=[L[B-1],L[B-2]]),L}function x(F){return P(F)>>1}function T(F){let B=F.length,L=0;for(let g=0;g<B;g++)L+=+(F[g]===1);let R=B-L;if(R===0||L===0||B===1)return 0;let l=m(B-1,L),U=0;for(let g=0;R>0&&L>0&&B>1;g++)B--,F[g]===0?l=l*--R/B:(U+=l,l=l*L--/B);return U}function W(F,B,L){let R=L-B,l=m(L-1,B),U=[],g=L-1;for(let A=0;A<g;A++)L--,F<l?(U.push(0),l=l*--R/L):(U.push(1),F-=l,l=l*B--/L);return U.push(B),U}function p(F,B){let L=[];for(let R=0;R<B.length;R++)L[R]=F[B[R]];return L}function i(F,B){let L=[];for(let R=0;R<B;R++)L[R]=R;for(let R=0;R<F.length;R++)L[F[R]]=F[(R+1)%F.length];return L}function f(F,B){let L=Array(B).fill(0);for(let R in F)L[R]=F[R];return L}function X(F,B){let L=Array(8).fill(0);for(let R=0;R<8;R++)L[R]=(F[1][R]+B[1][R])%3;return[p(F[0],B[0]),L]}var Y=[i([0,1,4],12),f({0:2},8)],Z=[i([1,2,5],12),f({1:2},8)],$=[i([2,3,6],12),f({2:2},8)],BB=[i([3,0,7],12),f({3:2},8)],LB=[i([9,8,4],12),f({4:2},8)],RB=[i([10,9,5],12),f({5:2},8)],FB=[i([11,10,6],12),f({6:2},8)],UB=[i([8,11,7],12),f({7:2},8)];Q(0,12),Array(8).fill(0);var s=[Y,Z,$,BB,LB,RB,FB,UB],lB=["UL","U","UR","F","L","B","R","D"],_=[0,1,0,1,1,0,1,0];function DB(F){let B=["0","","'"];return F.map(([R,l])=>lB[R]+B[l]).join(" ")}function gB(F){let B=V(Math.floor(F(h(12))/2),12),L=Array(8);for(let R=0;R<8;R++)L[R]=F(3);return[B,L]}function AB(F){return eB(gB(F))}function eB(F){let B=tB(F),L=[S(),j()],R=[nB(),oB()],l=[z(),aB()],U=[iB(),rB()],g=uB(B,L,R),A,D=new Set,r=+new Date;for(let o=0;o<25;o++){let{value:t,done:n}=g.next(),a=F;for(let[u,I]of t)for(let w=0;w<I;w++)a=X(a,s[u]);if(D.has(a.toString()))continue;D.add(a.toString());let O=x(a[0].slice(0,8)),d=0;for(let u=0;u<4;u++)d+=a[1][u]*3**u;let q=[O,d],G=A?A.length-t.length-1:999999,c=fB(q,l,U,G);if(c!==void 0&&((A===void 0||A.length>t.length+c.length)&&(A=t.concat(c)),new Date-r>300))break}return A}function tB(F){let B=0;for(let U=0;U<4;U++)B+=F[0].indexOf(U+8)*12**U;let L=0;for(let U=0;U<4;U++)L+=F[1][U+4]*3**U;let R=F[0].map(U=>Math.max(-1,U-8)),l=T(R.map(U=>+(U>=0)))*2+b(R.filter(U=>U>=0));return[B,L+81*l]}var e={};function M(){if(e.phase1cm)return e.phase1cm;let F=[];for(let B=0;B<81;B++){F[B]=Array(8);let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];F[B][0]=F[B][1]=F[B][2]=F[B][3]=B;for(let R=0;R<4;R++)L[R]=(L[R]+2)%3,F[B][4+R]=L[0]+L[1]*3+L[2]*9+L[3]*27,L[R]=(L[R]+1)%3}return e.phase1cm=F}function rB(){if(e.phase1cp)return e.phase1cp;let F=Array(81);for(let B=0;B<81;B++){let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];F[B]=(L[0]!==0)+(L[1]!==0)+(L[2]!==0)+(L[3]!==0)}return e.phase1cp=F}function S(){if(e.phase1em)return e.phase1em;let F=[];for(let L=0;L<12;L++){F[L]=[];for(let R=0;R<8;R++)F[L][R]=s[R][0].indexOf(L)}let B=Array(12**4);for(let L=0;L<12**4;L++){B[L]=Array(8);for(let R=0;R<8;R++){let l=0;for(let U=0;U<4;U++)l+=F[Math.floor(L/12**U)%12][R]*12**U;B[L][R]=l}}return e.phase1em=B}function nB(){return e.phase1ep?e.phase1ep:e.phase1ep=v(S(),[8+12*(9+12*(10+12*11))])}function j(){if(e.phase1sm)return e.phase1sm;const F=m(12,4);let B=[];for(let l=0;l<F;l++){B[l]=[];let U=W(l,4,12),g=[];for(let A=0,D=0;A<12;A++)U[A]===0?g[A]=-1:g[A]=D++;for(let A=0;A<8;A++){let D=p(g,s[A][0]),r=p(U,s[A][0]),o=b(D.filter(t=>t>=0));B[l][A]=T(r)*2+o}}let L=M(),R=[];for(let l=0;l<F;l++)for(let U=0;U<81;U++){let g=R[U+81*(2*l)]=[],A=R[U+81*(2*l+1)]=[];for(let D=0;D<8;D++)g[D]=L[U][D]+81*B[l][D],A[D]=L[U][D]+81*(B[l][D]^1)}return e.phase1sm=R}function oB(){return e.phase1sp?e.phase1sp:e.phase1sp=v(j(),[0])}function aB(){if(e.phase2cm)return e.phase2cm;let F=M(),B=Array(81);for(let L=0;L<81;L++)B[L]=F[L].slice(4,8);return e.phase2cm=B}function z(){if(e.phase2em)return e.phase2em;const F=8,B=h(F)/2;let L=Array(B),R=[0,1,2,3,4,5,6,7];for(let l=0;l<B;l++){L[l]=Array(4);for(let g=0;g<4;g++){let A=p(R,s[g][0].slice(0,8));L[l][g]=x(A)}if(l===B-1)break;let U=0;do for(let g=F-2;g>=0;g--){if(R[g]>R[g+1])continue;let A=g+1;for(let D=A;D<F;D++)R[D]>R[g]&&(A=D);[R[g],R[A]]=[R[A],R[g]],U^=1;for(let D=0;g+1+D<F-1-D;D++,U^=1)[R[g+1+D],R[F-1-D]]=[R[F-1-D],R[g+1+D]];break}while(U!==0)}return e.phase2em=L}function iB(){return e.phase2ep?e.phase2ep:e.phase2ep=v(z(),[0])}function v(F,B){let L=F.length,R=F[0].length,l=Array(L).fill(-1),U=B.slice(),g=[],A=0;for(;U.length>0;){g.length=0;for(let D of U)if(l[D]===-1){l[D]=A;for(let r=0;r<R;r++){let o=F[D][r];for(;o!=D;)g.push(o),o=F[o][r]}}[U,g]=[g,U],A+=1}return l}function fB(F,B,L,R){R=R||999999;let l=F.length,U=0;for(let g=0;g<l;g++)U=Math.max(U,L[g][F[g]]);for(;U<=R;){let g=k(F,B,L,U,-1);if(g!==void 0)return g;U++}}function k(F,B,L,R,l){let U=F.length,g=B[0][0].length,A=0;for(let D=0;D<U;D++)A=Math.max(A,L[D][F[D]]);if(!(A>R)){if(R===0)return[];if(!(A===0&&R===1))for(let D=0;D<g;D++){if(D===l||D<l&&_[D]==_[l])continue;let r=F.slice();for(let t=0;t<U;t++)r[t]=B[t][F[t]][D];let o=1;for(;F.some((t,n)=>F[n]!=r[n]);){let t=k(r,B,L,R-1,D);if(t!==void 0)return[[D,o]].concat(t);for(let n=0;n<U;n++)r[n]=B[n][r[n]][D];o++}}}}function*uB(F,B,L){let R=F.length,l=0;for(let U=0;U<R;U++)l=Math.max(l,L[U][F[U]]);for(;;)yield*C(F,B,L,l,-1),l++}function*C(F,B,L,R,l){let U=F.length,g=B[0][0].length,A=0;for(let D=0;D<U;D++)A=Math.max(A,L[D][F[D]]);if(!(A>R)){if(R===0){yield[];return}if(!(A===0&&R===1))for(let D=0;D<g;D++){if(D===l||D<l&&_[D]==_[l])continue;let r=F.slice();for(let t=0;t<U;t++)r[t]=B[t][F[t]][D];let o=1;for(;F.some((t,n)=>F[n]!=r[n]);){let t=C(r,B,L,R-1,D);for(;;){let{value:n,done:a}=t.next();if(a)break;yield[[D,o]].concat(n)}for(let n=0;n<U;n++)r[n]=B[n][r[n]][D];o++}}}}var sB=E();async function cB(){return new N(DB(AB(await sB)))}export{cB as getRandomRediCubeScramble,_B as sgsDataFTO};
